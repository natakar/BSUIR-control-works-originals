...
VAR
  stk: PType;
  input, input1, output: string;	// input1 – вводимое инфиксное выражение
...
BEGIN
  ...
  input := '';  
  Write('  Инфиксная форма: ');Readln(input1); writeln;  
  for i := length(input1) downto 1 do   //перепишем справа налево
    input := input + input1[i];
  writeln('  (Промежуточная Обратная форма: ', input, ' )'); 
  writeln;  writeln('  ------------'); writeln;
  i := 1;
  while i <= Length(input1) do //пока не дойдем до последнего элемента
    //Стек не пустой или приоритет символа стека больше, чем входного
    ...
    else    //Стек не пустой и приоритет входного символа меньше
    begin
  ...
  while not (stk = nil) do      //Дописываем всё что осталось в стеке
  begin
    Ch := GetStek(stk);              //Забираем из стека верхний символ
    if Ch <> '(' then                 // ( Добавлять в результат не надо
      output := output + Ch;          //Дописываем к выходной строке
  end;  
  Writeln('  Префиксная форма: ', output);
  writeln;
